...
extern char gamma_set_level_15[26];
extern char gamma_buffer[15][25];
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	...
	//if (!mfd->panel_power_on || !bl_updated) {   //20120702_ejkim EF44S PLM3232 FIX
			...
			unsigned long arg)
{
	...
	case MSMFB_SKY_LCD_FORCE_ONOFF:
		...
		case MSMFB_SKY_LCD_SMART_DIMMING_READ_GAMMA_300NIT:
			...
			if(copy_to_user(argp,gamma_set_level_15,sizeof(gamma_set_level_15))){
				printk(KERN_ERR "%s: copy2user failed \n", __func__);
				return -EFAULT;
			}
		        up(&msm_fb_ioctl_ppp_sem);
