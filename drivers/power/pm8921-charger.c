...
#elif defined(T_MAGNUS)
#define OEM_MAX17058_STD_FULL		(1018)
#define OEM_MAX17058_STD_EMPTY	(4) 
#define OEM_MAX17058_EXT_FULL		(990)
							...
							u8 mask, u8 val)
{
	...
	if (rc) {
		pr_debug("pm8xxx_writeb failed: addr=%03X, rc=%d\n", addr, rc);
		return rc;
	}
	return 0;
}
...
static int pm_chg_get_fsm_state(struct pm8921_chg_chip *chip)
{
	...
	if (err) {
		pr_debug("Error %d writing %d to addr %d\n", err, temp, CHG_TEST);
		return err;
	}
	...
	if (err) {
		pr_debug("pm8xxx_readb fail: addr=%03X, rc=%d\n", CHG_TEST, err);
		return err;
	}
...
static int pm_chg_vddmax_get(struct pm8921_chg_chip *chip, int *voltage)
{
	...
	if (rc) {
		pr_debug("rc = %d while reading vdd max\n", rc);
		*voltage = 0;
		return rc;
	}
...
static int pm_chg_vddmax_set(struct pm8921_chg_chip *chip, int voltage)
{
	...
	if (ret) {
		pr_debug("Failed to read vddmax rc=%d\n", ret);
		return -EINVAL;
	}
...
static int pm_chg_iterm_get(struct pm8921_chg_chip *chip, int *chg_current)
{
	...
	if (rc) {
		pr_debug("err=%d reading CHG_ITEM\n", rc);
		*chg_current = 0;
		return rc;
	}
...
static int pm_chg_iusbmax_get(struct pm8921_chg_chip *chip, int *mA)
{
	...
	*mA = 0;
	...
	if (rc) {
		pr_debug("err=%d reading PBL_ACCESS2\n", rc);
		return rc;
	}
...
static int pm_chg_vtrkl_low_set(struct pm8921_chg_chip *chip, int millivolts)
{
	...
	if (millivolts < PM8921_CHG_VTRKL_MIN_MV
		...
		pr_debug("bad voltage = %dmV asked to set\n", millivolts);
		return -EINVAL;
	}
...
static int pm_chg_itrkl_set(struct pm8921_chg_chip *chip, int milliamps)
{
	...
	if (milliamps < PM8921_CHG_ITRKL_MIN_MA
		...
		pr_debug("bad current = %dmA asked to set\n", milliamps);
		return -EINVAL;
	}
...
static void disable_input_voltage_regulation(struct pm8921_chg_chip *chip)
{
	...
	if (rc) {
		pr_debug("Failed to write 0x70 to CTRL_TEST3 rc = %d\n", rc);
		return;
	}
	rc = pm8xxx_readb(chip->dev->parent, CHG_BUCK_CTRL_TEST3, &temp);
	if (rc) {
		pr_debug("Failed to read CTRL_TEST3 rc = %d\n", rc);
		return;
	}
	...
	if (rc) {
		pr_debug("Failed to write 0x%x to CTRL_TEST3 rc=%d\n", temp, rc);
		return;
	}
}
...
static int is_battery_valid(struct pm8921_chg_chip *chip)
{
	...
	if (rc < 0) {
		pr_debug("error reading batt id channel = %d, rc = %lld\n",
					chip->vbat_channel, rc);
	...
	if (rc < chip->batt_id_min || rc > chip->batt_id_max) {
		pr_debug("batt_id phy =%lld is not valid\n", rc);
		return 0;
	}
	return 1;
}
...
static void check_battery_valid(struct pm8921_chg_chip *chip)
{
	if (is_battery_valid(chip) == 0) {
		pr_debug("batt_id not valid, disbling charging\n");
...
static int charger_set_battery_data(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("pm8921-charger: battery id (%lld)\n", battery_id);
	...
	if (is_between(BATTERY_ID_DUMMY_MIN, BATTERY_ID_DUMMY_MAX, battery_id)) {
		...
		if(!is_dummy_battery) {
			...
			pr_debug("battery is dummy (%lld)!!!\n", battery_id);
		}
	} else {
		if (is_dummy_battery) {
			is_dummy_battery = false;
			pr_debug("battery isn't dummy (%lld)!!!\n", battery_id);
		}
	}
	#endif
			...
			if (end == CHG_NOT_IN_PROGRESS ||end == CHG_FINISHED) {
				if (is_usb_chg_plugged_in(chip) ||is_dc_chg_plugged_in(chip)) {
					...
					if (vbat_meas_uv >= 3800000) {
						pr_debug("battery charge is done in abnormal temp\n");
...
#endif
		pr_debug("Unkown POWER_SUPPLY_TYPE %d\n", psy->type);
		break;
#if defined(CONFIG_PANTECH_CHARGER_WIRELESS)
...
static int get_prop_batt_current(struct pm8921_chg_chip *chip)
{
	...
	if (rc) {
		pr_debug("unable to get batt current rc = %d\n", rc);
		return rc;
	} else {
		return result_ua;
	}
...
static int get_prop_batt_fcc(struct pm8921_chg_chip *chip)
{
	...
	if (rc < 0)
		pr_debug("unable to get batt fcc rc = %d\n", rc);
	return rc;
}
...
static int get_prop_batt_status(struct pm8921_chg_chip *chip)
{
	...
	if (pm_chg_get_rt_status(chip, BATTTEMP_HOT_IRQ) || 
		...
		pr_debug("battery temperature is too hot or cold\n");
		batt_state = POWER_SUPPLY_STATUS_DISCHARGING;
	} else {
		int ov;
...
static int get_prop_batt_temp(struct pm8921_chg_chip *chip)
{
...
#if defined(FEATURE_PANTECH_BATTERY_DUMMY)
	if (is_dummy_battery) {
		pr_debug("Dummy battery recognized\n");
		return 300;
	}
#endif
	...
	pr_debug("battery temperature = %d\n", chip->current_temp/10);
#endif

	if (result.physical > MAX_TOLERABLE_BATT_TEMP_DDC)
		pr_debug("BATT_TEMP= %d > 68degC, device will be shutdown\n",
							(int) result.physical);
...
static int init_charger(struct pm8921_chg_chip *chip)
{
	...
	if (ret)
		pr_debug("Failed to write CHG_FAILED_CLEAR bit\n");
	...
	if (ret)
		pr_debug("Failed to set max time to %d minutes ret=%d\n", chip->safety_time, ret);
...
static uint get_cable_type(struct pm8921_chg_chip *chip)
{
	...
	pr_debug("is_usb : %d, is_dc : %d, USB_IN adc : %lld, chg_usb_type : %d\n", is_usb, is_dc, adc_val, chg_usb_type);
...
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	if (!the_chip) {
		pr_debug("chip not yet initalized\n");
		return;
	}
	...
	if (!get_prop_batt_present(the_chip)
		...
		pr_debug("rejected: no other power source connected\n");
		return;
	}
...
int pm8921_usb_ovp_set_threshold(enum pm8921_usb_ov_threshold ov)
{
	...
	if (ov > PM_USB_OV_7V) {
		pr_debug("limiting to over voltage threshold to 7volts\n");
		ov = PM_USB_OV_7V;
	}
...
int pm8921_usb_ovp_set_hystersis(enum pm8921_usb_debounce_time ms)
{
	...
	if (ms > PM_USB_DEBOUNCE_80P5MS) {
		pr_debug("limiting debounce to 80.5ms\n");
...
static void turn_off_usb_ovp_fet(struct pm8921_chg_chip *chip)
{
	...
	if (rc) {
		pr_debug("Failed to write 0x30 to USB_OVP_TEST rc = %d\n", rc);
		return;
	}
	rc = pm8xxx_readb(chip->dev->parent, USB_OVP_TEST, &temp);
	if (rc) {
		pr_debug("Failed to read from USB_OVP_TEST rc = %d\n", rc);
		return;
	}
	...
	if (rc) {
		pr_debug("Failed to write 0x%x USB_OVP_TEST rc=%d\n", temp, rc);
		return;
	}
}
...
static void turn_on_usb_ovp_fet(struct pm8921_chg_chip *chip)
{
	...
	rc = pm8xxx_writeb(chip->dev->parent, USB_OVP_TEST, temp);
	if (rc) {
		pr_debug("Failed to write 0x%x to USB_OVP_TEST rc = %d\n",
								temp, rc);
		return;
	}
}
...
//        }
		...
		pr_debug("cable_type=%d, iusb_imax=%d, ibat_imax=%d\n", cable_type, iusbmax_set_cur, ibatmax_set_cur);
...
static int is_charging_finished(struct pm8921_chg_chip *chip)
{
	...
	if (!is_ext_charging(chip)) {
		...
		if (rc) {
			pr_debug("couldnt read vddmax rc = %d\n", rc);
			return CHG_IN_PROGRESS;
		}
		...
		if (regulation_loop < 0) {
			pr_debug("couldnt read the regulation loop err=%d\n",
				regulation_loop);
			return CHG_IN_PROGRESS;
		}
	...
	if (rc) {
		pr_debug("couldnt read iterm rc = %d\n", rc);
		return CHG_IN_PROGRESS;
	}
...
static void btm_configure_work(struct work_struct *work)
{
	...
	if (rc)
		pr_debug("failed to configure btm rc=%d", rc);
}
...
static int configure_btm(struct pm8921_chg_chip *chip)
{
	...
	rc = pm8xxx_adc_btm_configure(&btm_config);
	if (rc)
		pr_debug("failed to configure btm rc = %d\n", rc);
	rc = pm8xxx_adc_btm_start();
	if (rc)
		pr_debug("failed to start btm rc = %d\n", rc);
					...
					struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(chg_irq_data); i++) {
		...
		if (ret < 0) {
			pr_debug("couldn't request %d (%s) %d\n", res->start,
...
static int __devinit pm8921_chg_hw_init(struct pm8921_chg_chip *chip)
{
	...
	if (rc) {
		pr_debug("Failed to set BOOT_DONE_BIT rc=%d\n", rc);
		return rc;
	}
	...
	if (rc) {
		pr_debug("Failed to set safe voltage to %d rc=%d\n",
	...
	if (rc) {
		pr_debug("Failed to set vbatdet comprator voltage to %d rc=%d\n",
	...
	if (rc) {
		pr_debug("Failed to set max voltage to %d rc=%d\n",
						chip->max_voltage_mv, rc);
		return rc;
	}
	...
	if (rc) {
		pr_debug("Failed to set max current to 400 rc=%d\n", rc);
		return rc;
	}

	rc = pm_chg_iterm_set(chip, chip->term_current);
	if (rc) {
		pr_debug("Failed to set term current to %d rc=%d\n",
	...
	rc = pm_chg_masked_write(chip, PBL_ACCESS2, ENUM_TIMER_STOP_BIT,
			ENUM_TIMER_STOP_BIT);
	if (rc) {
		pr_debug("Failed to set enum timer stop rc=%d\n", rc);
		return rc;
	}
...
#endif
	if (rc) {
		pr_debug("Failed to set usb max to %d rc=%d\n", 0, rc);
		return rc;
	}
	...
	if (chip->ttrkl_time != 0) {
		...
		if (rc) {
			pr_debug("Failed to set trkl time to %d minutes rc=%d\n",
							chip->safety_time, rc);
			return rc;
		}
	}
	...
	if (chip->vin_min != 0) {
		...
		if (rc) {
			pr_debug("Failed to set vin min to %d mV rc=%d\n",
							chip->vin_min, rc);
			return rc;
		}
	} else {
	...
	if (rc) {
		pr_debug("Failed to disable wd rc=%d\n", rc);
		return rc;
	}
	...
	if (chip->trkl_voltage != 0) {
		...
		if (rc) {
			pr_debug("Failed to set trkl voltage to %dmv  rc=%d\n",
	...
	if (chip->weak_voltage != 0) {
		...
		if (rc) {
			pr_debug("Failed to set weak voltage to %dmv  rc=%d\n",
	...
	if (chip->trkl_current != 0) {
		...
		if (rc) {
			pr_debug("Failed to set trkl current to %dmA  rc=%d\n",
							chip->trkl_voltage, rc);
			return rc;
		}
	}
	...
	if (chip->weak_current != 0) {
		...
		if (rc) {
			pr_debug("Failed to set weak current to %dmA  rc=%d\n",
	...
	if (rc) {
		pr_debug("Failed to set cold config %d  rc=%d\n",
						chip->cold_thr, rc);
	}

	rc = pm_chg_batt_hot_temp_config(chip, chip->hot_thr);
	if (rc) {
		pr_debug("Failed to set hot config %d  rc=%d\n",
						chip->hot_thr, rc);
	}
	...
	if (rc)
		pr_debug("Failed to Force Vref therm rc=%d\n", rc);

	rc = pm_chg_charge_dis(chip, charging_disabled);
	if (rc) {
		pr_debug("Failed to disable CHG_CHARGE_DIS bit rc=%d\n", rc);
		return rc;
	}
...
static int get_reg(void *data, u64 * val)
{
	...
	if (ret) {
		pr_debug("pm8xxx_readb to %x value =%d errored = %d\n",
			addr, temp, ret);
		return -EAGAIN;
	}
	*val = temp;
...
static int set_reg(void *data, u64 val)
{
	...
	if (ret) {
		pr_debug("pm8xxx_writeb to %x value =%d errored = %d\n",
			addr, temp, ret);
		return -EAGAIN;
	}
	return 0;
...
static void create_debugfs_entries(struct pm8921_chg_chip *chip)
{
	...
	if (IS_ERR(chip->dent)) {
		pr_debug("pmic charger couldnt create debugfs dir\n");
		return;
	}
...
#endif
	if (rc)
		pr_debug("Failed to Force Vref therm off rc=%d\n", rc);
...
static int pm8921_charger_resume_noirq(struct device *dev)
{
	...
	rc = pm_chg_masked_write(chip, CHG_CNTRL, VREF_BATT_THERM_FORCE_ON,
						VREF_BATT_THERM_FORCE_ON);
	if (rc)
		pr_debug("Failed to Force Vref therm on rc=%d\n", rc);
	return 0;
}
...
static int pm8921_charger_resume(struct device *dev)
{
	...
	if (!(chip->cool_temp_dc == INT_MIN && chip->warm_temp_dc == INT_MIN)
		...
		rc = pm8xxx_adc_btm_configure(&btm_config);
		if (rc)
			pr_debug("couldn't reconfigure btm rc=%d\n", rc);

		rc = pm8xxx_adc_btm_start();
		if (rc)
			pr_debug("couldn't restart btm rc=%d\n", rc);
	}
...
static int pm8921_charger_suspend(struct device *dev)
{
	...
	if (!(chip->cool_temp_dc == INT_MIN && chip->warm_temp_dc == INT_MIN)
		...
		if (rc)
			pr_debug("Failed to disable BTM on suspend rc=%d\n", rc);
	}
		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	case PM8921_CHARER_TEST_SET_CHARING_CTL:
		...
		if (!rc) {
			...
			pr_debug("SET_CHARING_CTL charging_disabled[%d]\n",charging_disabled);
		}
	...
	case PM8921_CHARGER_TEST_SET_PM_CHG_TEST:
		...
		if (!rc) {
			...
			pr_debug("SET_PM_CHG_TEST pm_chg_test [%d]\n",the_chip->pm_chg_test);
		}	
		break;
#endif
...
void pm8921_charger_test_charger_monitor_init(struct pm8921_chg_chip *chip)
{
	...
	if (!pm8921_charger_dir) {
		pr_debug("Unable to create /proc/%s directory\n",PM8921_CHARGER_DEV_NAME);
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/fsm_state entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/cable_type entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/cable_id entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/batt_id entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/i_usbmax entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/i_battmax entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/i_battcurr entry\n");
		return;
	}
	...
	if (!ent) {
		pr_debug("Unable to create /proc/pm_chg_test entry\n");
		return;
	}
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	if (!pdata) {
		pr_debug("missing platform data\n");
		return -EINVAL;
	}
	...
	chip = kzalloc(sizeof(struct pm8921_chg_chip),
					GFP_KERNEL);
	if (!chip) {
		pr_debug("Cannot allocate pm_chg_chip\n");
		return -ENOMEM;
	}
	...
	if (smem_id_vendor1_ptr->battery_id == OEM_PM_BATTERY_TYPE_DUMMY) {
		is_dummy_battery = true;
		pr_debug("the dummy battery is insered !!!\n");
	}
#endif
	...
	if (i2c_add_driver(&smb347_i2c_driver))
		pr_debug("Can't add smb347 i2c drv\n");
#endif
#if defined(CONFIG_PANTECH_PMIC_MAX17058)
	#if defined(T_EF44S)
	#if(BOARD_VER >= WS20)
	...
	if (max17058_uses)
		if (i2c_add_driver(&max17058_i2c_driver))
			pr_debug("Can't add max17058 i2c drv\n");
#endif // #if defined(CONFIG_PANTECH_PMIC_MAX17058)
...
//#endif
	...
	if (rc < 0) {
		pr_debug("power_supply_register usb failed rc = %d\n", rc);
		goto free_chip;
	}
	...
	if (rc < 0) {
		pr_debug("power_supply_register batt failed rc = %d\n", rc);
		goto unregister_dc;
	}
	...
	if (rc) {
		pr_debug("couldn't register interrupts rc=%d\n", rc);
	...
	if (!(chip->cool_temp_dc == INT_MIN && chip->warm_temp_dc == INT_MIN)) {
		...
		if (rc) {
			pr_debug("couldn't register with btm rc=%d\n", rc);
			goto free_irq;
		}
	}
